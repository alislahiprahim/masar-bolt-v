...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, TranslatePipe, FontAwesomeModule, ImgUrlPipe, RouterLink],
  template: `
                  ...
                  <div class="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent">
                    <div class="absolute bottom-0 left-0 right-0 p-6">
                      ...
                      @if (city._count.trips > 0) {
                        <p class="text-white/90">{{ city._count.trips }} trips available</p>
                      }
                    </div>
                  </div>
                </div>
...
export class DestinationsSectionComponent implements OnInit {
  ...
  protected getDestinationTripsCount(destination: string): number {
    return this.state.cities().filter(city => city.name === destination).length;
  }
}

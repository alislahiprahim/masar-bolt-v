...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, TranslateModule, FaIconComponent, RouterLink, QRCodeComponent],
  template: `
      ...
      <div class="p-6">
        ...
        <div class="space-y-3 mb-6">
          <div class="flex justify-between text-sm">
            <span class="text-gray-600">{{ 'reservations.travelers' | translate }}</span>
          ...
          <div *ngIf="reservation.tripHotel" class="flex justify-between text-sm">
            <span class="text-gray-600">{{ 'booking.Hotel' | translate }}</span>
            <span class="font-medium">{{ reservation.tripHotel.hotel?.name }}</span>
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-gray-600">{{ 'reservations.totalPrice' | translate }}</span>
            <span class="font-medium">{{ reservation.totalPrice | currency }}</span>
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-gray-600">{{ 'reservations.date' | translate }}</span>
        ...
        <div class="flex justify-between items-center">
          ...
          <div class="flex flex-row gap-2">
            @if (reservation.status === 'PAID' || reservation.status === 'COMPLETED') {
              <button
                (click)="printService.exportTicket(reservation, this.qrcode(), 'download')"
    ...
    <div class="qrcodeImage">
      <qrcode
        #qrcode
        [qrdata]="environment.apiUrl + '/reservations/' + reservation.id + '/paid'"
